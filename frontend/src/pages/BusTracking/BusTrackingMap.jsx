...
const BusTrackingMap = () => {
  ...
  const updateBusInfo = (busId, lat, lng, passengers, nextStop) => {
    ...
    let speed = 0;
    if (previousLocation) {
      const distance = calculateDistance(previousLocation.lat, previousLocation.lng, lat, lng);
          ...
          {isLoading ? (
            <div className="w-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="mt-4 text-gray-600">Connecting to the bus tracking system...</p>
              </div>
            </div>
          ) : (
            <>
                  ...
                  <CardContent>
                    {selectedBus ? (
                      <div className="space-y-4">
                        <h3 className="text-lg font-semibold">Bus {selectedBus}</h3>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <p className="text-sm text-gray-500">Speed</p>
                            <p className="font-medium">{formatSpeed(busLocations[selectedBus].speed)}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500">Passengers</p>
                            <p className="font-medium">{busLocations[selectedBus].passengers}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500">Next Stop</p>
                            <p className="font-medium">{busLocations[selectedBus].nextStop}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500">ETA</p>
                            <p className="font-medium">
                              {formatTime(calculateETA(1000, busLocations[selectedBus].speed))}
                            </p>
                          </div>
                        </div>
                        <Badge 
                          variant={busLocations[selectedBus].speed > 0 ? "success" : "warning"}
                        >
                          {busLocations[selectedBus].speed > 0 ? "Moving" : "Stopped"}
                        </Badge>
                        <Button onClick={() => setSelectedBus(null)} className="w-full">
                          Clear Selection
                        </Button>
                      </div>
                    ) : (
                      <p className="text-gray-500">Select a bus on the map to view details</p>
                    )}
                  </CardContent>
                </Card>
                  ...
                  <CardContent>
                    <ul className="space-y-2">
                      {filteredBuses.map(([busId, bus]) => (
                        <li key={busId}>
                          <Button variant="outline" onClick={() => setSelectedBus(busId)} className="w-full justify-between">
                            <span>Bus {busId}</span>
